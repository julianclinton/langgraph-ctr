...
#     Each agent calls .invoke() directly on its tools. This beginner-friendly approach makes the flow explicit and easy to follow.
#     Each agent returns only the messages key — LangGraph merges this update into the full state automatically.
#     Every agent reads file_path from the shared state, so all agents always know where the data lives.

class AgentState(TypedDict):
    messages: Annotated[Sequence[str], operator.add]
    file_path: str

# The EDA (Exploratory Data Analysis) Expert handles all data preprocessing.
# It runs both the encoding and standardization tools in sequence before passing control to the next agent.
...
# The Statistician trains the model using the preprocessed data produced by the EDA Agent.
...
# The Visualization Expert plots the model's predictions.
# This node runs last, once the model has been trained and predictions have been saved.
